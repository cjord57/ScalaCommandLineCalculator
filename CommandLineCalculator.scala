/*
Corey Jordan-Zamora
Command-Line Calculator Extra Credit Assignment
*/
...
object interpreter:
  ...
  def evaluate(env: Env, e: Exp): Val =
    ...
    // TODO: Add case for EIf below.
      case EIf(b, tt, ff) =>
        val bVal = evaluate(env, b)
        bVal match {
          case VInt(0) => evaluate(env, ff)
  ...
  // TODO: Add case for EIf below.
    case EIf(b, tt, ff) =>
      s"(if ${prettyPrintExp(b)} then ${prettyPrintExp(tt)} else ${prettyPrintExp(ff)})"
  ...
  // TODO: Change the body of the definition below.
  val exXPlusFifty: Exp = EApp(EApp(EVal(primAdd), EVar("x")), EVal(VInt(50)))
  ...
  // TODO: Change the body of the definition below.
  val exXFunPlusFifty: Exp = EAbs("x", EApp(EApp(EVal(primAdd), EVar("x")), EVal(VInt(50))))
  ...
  // TODO: Change the body of the definition below.
  val exXFunPlusFiftyApplied: Exp = EApp(exXFunPlusFifty, EVal(VInt(30)))
  ...
  // TODO: Change the body of the definition below.
  val exLetXLetY: Exp = ELet("x", EVal(VInt(5)), ELet("y", EVal(VInt(6)), EApp(EApp(EVal(primAdd), EVar("x")), EVar("y"))))

  // EXERCISE 8: write abstract syntax corresponding to:
  //   let f = ( (x:Int) => x + 50 );
  //   f (5)
  // TODO: Change the body of the definition below.
  val exLetF: Exp = ELet("f", exXFunPlusFifty, EApp(EVar("f"), EVal(VInt(5))))
  ...
  // TODO: Change the body of the definition below.
  val exPair: Exp = EPair(EVal(VInt(5)), EAbs("x", EVal(VInt(6))))
  ...
  // TODO: Change the body of the definition below.
  val exCounter: Exp = EFst(EApp(ESnd(EApp(counter, EVal(VNil))), EVal(VNil)))
  // All the solutions I tried that didn't work:
  //EFst(ESnd(EVar("counter")))
  //EFst(ESnd(EFst(EVar("counter"))))
  //EFst(ESnd(EApp(EVar("counter"), EVal(VNil))))
  //EFst(ESnd(EApp(EFst(EVar("counter")), EVal(VNil))))
  //ELet("counter", EApp(counter, EVal(VNil)), ESnd(EVar("counter")))
  //ESnd(EFst(EApp(counter, EVal(VNil))))
  // Close, as dif error msg: //EFst(EApp(ESnd(EVar("counter")), EVal(VNil)))
  // Did not match: //EFst(EApp(EApp(ESnd(EVar("counter")), EVal(VInt(4))), EVal(VNil)))
  //EFst(EPair(ESnd(EVar("counter")), EVal(VInt(4))))
  //EFst(EApp(ESnd(EApp(counterFunction, EVal(VInt(4)))), EVal(VNil)))
  //EFst(EApp(EApp(ESnd(counterFunction), EVal(VInt(4))), EVal(VNil)))
  //EFst(ESnd(EApp(counterFunction, EVal(VInt(4)))))
  //EFst(ESnd(EApp(counter, EVal(VNil))))
  //EFst(ESnd(EApp(EVar("counterFunction"), EVal(VInt(4)))))
  //EFst(EApp(ESnd(EApp(EVar("counterFunction"), EVal(VInt(4)))), EVal(VNil)))
  ...
  /*
  If we were to implement functions naively, without using closures, the counterFunction would not be able to maintain its internal state (whatever the current number is) 
  between function calls. This is because each function call would create a new scope, and the variables would be reinitialized.
  So with a naive implementation, let me explain, in steps, about what would happen in more detail:
  [Explanation steps]:
    1) counterFunction is called with 4 as an argument, and it returns a pair containing 4 and a new function.
    2) The new function is extracted and called with Nil as an argument. However, since the function is not a closure, it does not remember what the prev state (the number 4).
    3) The function is reinitialized, and it starts from scratch, returning a pair containing 4 again, instead of 5.
    4) The process repeats, and the function never properly increments the number.
  So by using closures, we can capture the current state of the function (the number) and preserve it (keep the right number) between function calls. 
  This allows the function to maintain its internal state and behave correctly.
  Therefore, in the correct implementation, such as what I did for Exercise 11, when counterFunction is called, it returns a closure that captures the current number. 
  When the closure is called again, it remembers the previous state and increments the number correctly (+1 to the correctly remembered/kept prev number). 
  */
